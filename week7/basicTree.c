...
tree_t* searchXX(tree_t *nodest,int value){	
	...
	//printf("=1...%d %d...\n",nodest->value,value);
	if( nodest->value == value )	return nodest;
	//printf("=2 %p\n",nodest->first_child);
	...
	if( to != NULL) {
		...
		//printf("=5\n");
		return searchXX(nodest->next_sibling,value);
	}
}

int search(tree_t *nodest,int value){	
	tree_t* to = searchXX(nodest,value);
	if(to==NULL)	return 0;
	return 1;
}
...
tree_t* attach(tree_t *nodest,int parent,int child){
	...
	if(to->first_child==NULL){
		to->first_child = new_node;	//printf("to->first_child - data of %d = %d\n",to->value,child);
	}
	else{
		to = to->first_child;
		while(to->next_sibling!=NULL)	to = to->next_sibling;
		to->next_sibling = new_node;	//printf("to->next_sibling - data of %d = %d\n",to->value,child);
	}
	//printf("3...\n");
	return nodest;
}
...
stack_t* searchprint_path(stack_t* s,tree_t *nodest,int value){	
	...
	if( to != NULL) {
		//printf("test2 = %d \n",nodest->value);
		to = push(to,nodest->value);
		return to;
	}
	else			{
		return searchprint_path(to,nodest->next_sibling,value);
	}
}
